...
export async function POST(request) {
  ...
  try {
    ...
    const claim_sPoints = parseInt(process.env.FOLLOW_POINTS|| '500', 10);



    // Check if follow_spoints already have 500 points
    if (user.follow_spoints === claim_sPoints) {
      return NextResponse.json({ message: 'You have already claimed these sPoints', user }, { status: 200 });
    }

    // If follow_spoints is zero, add 500 points to both follow_spoints and total_spoint
    if (user.follow_spoints === 0) {
      user.follow_spoints = claim_sPoints;
    ...
    // If follow_spoints already have some points but less than 500
    return NextResponse.json({ message: 'You have already claimed part of these points', user }, { status: 200 });
    
  } catch (error) {
    console.error('Error updating follow points:', error);
